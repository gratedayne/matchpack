...
export class PartitionPackingSolver extends BaseSolver {
  ...
  private applyPackingResult(
    ...
    const decouplingCaps = this.packedPartitions.filter(p => p.inputProblem.chipMap && Object.values(p.inputProblem.chipMap).some(c => c.type === 'capacitor' && c.footprint === '0402' && c.capacitance && parseFloat(c.capacitance) >= 100e-9));
    for (const capPartition of decouplingCaps) {
      const cap = Object.values(capPartition.inputProblem.chipMap).find(c => c.type === 'capacitor');
      if (!cap) continue;
      const chipId = Object.keys(capPartition.inputProblem.chipMap).find(id => capPartition.inputProblem.chipMap[id] === cap);
      if (!chipId) continue;
      const nearbyChipPartition = this.packedPartitions.find(p => p.inputProblem.netConnMap && Object.keys(p.inputProblem.netConnMap).some(conn => conn.includes('net.VCC') || conn.includes('net.GND')));
      if (nearbyChipPartition) {
        const chipPlacement = Object.values(nearbyChipPartition.layout.chipPlacements)[0] || { x: 0, y: 0 };
        const capPos = { x: chipPlacement.x + 2, y: chipPlacement.y - 2 }; // Near chip, short traces <5mm
        newChipPlacements[chipId] = {
          x: capPos.x,
          y: capPos.y,
          ccwRotationDegrees: capPartition.inputProblem.netConnMap && Object.keys(capPartition.inputProblem.netConnMap).some(conn => conn.includes('net.VCC')) ? 0 : 180 // Up for VCC (0 deg), down for GND (180 deg)
        };
      } else {
        // Cluster in PowerDist
        newChipPlacements[chipId] = {
          x: 0,
          y: -15,
